...
Platform-specific binary package for OMA CLI (macOS x64).

This package is part of the `@oma/cli` distribution. It contains the precompiled
`oma` binary for macOS on Intel (x64).

## How it works

At release time, GoReleaser builds the `oma` binary for darwin/amd64 and copies
